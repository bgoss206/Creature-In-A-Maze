...
- google-runtime-references: disabled because it is prompting for all const references and functions but the path and maze need to be modified
- readability-else-after-return: In my recursive step, I need to check for a valid path in a certain but otherwise move the creature back so I need else{} after return statement
- cppcoreguidelines-pro-bounds-constant-array-index: this is warning of using array subscript within given code to access elements of the maze
- cppcoreguidelines-pro-type-member-init: warning of maze constructor not initializing values but the given maze code intializes using the format-> InFile << (variableName) 
- hicpp-member-init: (same as previous diable) warning of maze constructor not initializing values but the given maze code intializes using the format-> InFile << (variableName) 
